	...
	<div class="content">
		<section class="grey lighten-4 py-2 d-print-none">
			<div class="container-fluid">
				...
				<h1 class="h5 font-weight-bold mb-0">Update Event Details</h1>
			</div>
		</section>
		...
		<section class="py-3">
			<div class="container-fluid">
				...
				<form name="form_event_edit" id="form_event_edit" method="post" action="#">
					...
					<div class="row">
						<div class="col-lg-3 col-sm-6">
							<div class="form-outline mb-3">
								<input type="text" name="startdate" id="startdate" class="form-control" value="<?= $event['startdate']; ?>" required />
								<label class="form-label">Start Date *</label>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="form-outline mb-3">
								<input type="text" name="starttime" id="starttime" class="form-control" value="<?= $event['starttime']; ?>" />
								<label class="form-label">Start Time *</label>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="form-outline mb-3">
								<input type="text" name="enddate" id="enddate" class="form-control" value="<?= $event['enddate']; ?>" required />
								<label class="form-label">End Date *</label>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="form-outline mb-3">
								<input type="text" name="endtime" id="endtime" class="form-control" value="<?= $event['endtime']; ?>" />
				...
				<?php } else { ?>
					<p class="h4 text-danger">Event Details Not Found</p>
				<?php } ?>
			</div>
		</section>
...
$(document).ready(function(e) {
	...
	$("form#form_event_edit input[name='startdate']").datepicker({ format: "yyyy-mm-dd", autoclose: true, todayHighlight: true })
	.on('changeDate', function(){
		...
		$("form#form_event_edit input[name='enddate']").datepicker('setStartDate', new Date($(this).val()));
	});
	$("form#form_event_edit input[name='enddate']").datepicker({ format: "yyyy-mm-dd", autoclose: true, todayHighlight: true })
	.on('changeDate', function(){
		document.querySelectorAll('.form-outline').forEach((formOutline) => { new mdb.Input(formOutline).init();
			($(formOutline).find('input').attr('type') == 'file' ? $(formOutline).find('input').addClass('active') : '');
		});
		$(this).valid();
		$("form#form_event_edit input[name='startdate']").datepicker('setEndDate', new Date($(this).val()));
	});
	//$('table#eventlist');
	$('form#form_event_edit').validate({
		//debug: true,
		onclick: false,
		...
		submitHandler: function(form){	var gg = {}; var validator = this;
			$(form).find('button:submit').prop({'disabled':true}).html('<i class="spinner-border spinner-border-sm"></i> Updating...');
			setTimeout(() => {
				var fd = new FormData(form);
				var returns = formsubmission({"method":"POST", "target":'<?= base_url('admin/event/update/'.$event['code']); ?>', "formdata":fd });
				$(form).find('button:submit').prop({'disabled':false}).html('Update');
