	...
	<div class="content">
		<section class="grey lighten-4 py-2 d-print-none">
			<div class="container-fluid">
				<a href="<?= base_url('user/dashboard'); ?>" class="btn btn-primary p-1 float-end event_add"><i class="fas fa-reply"></i> Back to List</a>
...
$(document).ready(function(e) {
	...
	// 	modalprocess({"affect":"common_modal", "title":"Add New Event Details", "style":"modal-xl modal-dialog-centered modal-dialog-scrollable modal-fullscreen-md-down", "target":"<?= base_url('user/event/add/form/'); ?>" });
	...
	$('form#form_event_add').validate({
		...
		submitHandler: function(form){	var gg = {}; var validator = this;
			...
			setTimeout(() => {
				...
				var returns = formsubmission({"method":"POST", "target":'<?= base_url('user/event/update/'.$event['code']); ?>', "formdata":fd });
				$(form).find('button:submit').prop({'disabled':false}).html('Update');
				
				if(returns.access == false) { window.location.replace("<?= base_url('user'); ?>"); }
				else if(returns.status == false) { notification({ 'type': 'danger', 'position': 'bottom-center', 'timer':5, 'title':returns.title, 'msg': returns.msg }); }
				else { window.location.replace("<?= base_url('user/dashboard'); ?>"); }
			}, 1);
			return false;
		}
	});
