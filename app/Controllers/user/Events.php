...
class Events extends BaseController {
	...
	public function Addform() { $profile = $this->Authenticate();
		...
		else { return view('user/event/addform', ['profile'=>$profile['profile'], 'alert'=>$this->alert()]); }
	}
	...
	public function Addnew() { $auth = $this->Authenticate();
		if($auth['status']) {
			...
			if($this->validation->withRequest($this->request)->run()) {
				$input['usercode'] = $auth['profile']['code'];
	...
	public function Editform($code) { $profile = $this->Authenticate();
		if($profile['status'] != TRUE) { return redirect()->to(base_url('user')); exit(0); }
			...
			return view('user/event/editform', ['profile'=>$profile['profile'], 'event'=> $event, 'alert'=>$this->alert()]);
		}
	}
